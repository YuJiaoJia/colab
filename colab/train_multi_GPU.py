...
def main(args):
    ...
    train_dataset = Dataset(args.data_path, train=True, transforms=SODPresetTrain([190, 280], crop_size=190))
    val_dataset = Dataset(args.data_path, train=False, transforms=SODPresetEval([190, 280]))
...
if __name__ == "__main__":
    ...
    # 训练文件的根目录(VOCdevkit)
    parser.add_argument('--data-path', default='./', help='train')
    # 训练设备类型
    ...
    # 文件保存地址
    parser.add_argument('--output-dir', default='./multi_save_weights', help='path where to save')
    # 基于上次的训练结果接着训练
