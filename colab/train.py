...
from  src import u2net_lite,u2net_litte,u2net_lite_half_m32\
                ,u2net_lite_half_m64,u2net_lite_half_m128,u2net_lite_half_m48
from train_utils import train_one_epoch, evaluate, get_params_groups, create_lr_scheduler
from train_utils.train_and_eval import EarlyStopping
...
def main(save_weights,mode,args):
    ...
    val_dataset = Dataset(args.data_path, train=False, transforms=SODPresetEval([200, 200]))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # 调用 EarlyStopping
        # early_stopping(mean_loss, save_weights, mode, save_file,f1_info,mae_info)
        ...
        if epoch % args.eval_interval == 0 or epoch == args.epochs - 1:
            ...
            print("########")
            print(f"[epoch: {epoch}] val_MAE: {mae_info:.3f} val_maxF1: {f1_info:.4f} precision: {precision:.3f} recall:{recall:.3f}")
            print("########")
            # write into txt
            with open(results_file, "a") as f:
                # 记录每个epoch对应的train_loss、lr以及验证集各指标
                write_info = f"[epoch: {epoch}] train_loss: {mean_loss:.4f} lr: {lr:.6f} " \
                             f"MAE: {mae_info:.3f} maxF1: {f1_info:.3f} recall:{recall:.3f} precision:{precision:.3f}\n"
                f.write(write_info)
                #tensorboard display
